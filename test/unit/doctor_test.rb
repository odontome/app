...
class DoctorTest < ActiveSupport::TestCase  
  ...
  test "doctor attributes must not be empty" do
 	...
 	test "doctor is not valid without a valid email address" do
 		...
 		assert !doctor.save
 		assert_equal I18n.translate("errors.messages.invalid"), doctor.errors[:email].join("; ")
 	end
 	...
 	test "doctor is not valid without an unique uid" do	
 		...
 		assert !doctor.save
 		assert_equal I18n.translate("activerecord.errors.messages.taken"), doctor.errors[:uid].join("; ")
 	end
 	...
 	test "doctor uid must be between 0 and 25 characters" do
 		...
 		assert !doctor.save
 		assert_equal I18n.translate("errors.messages.too_long", :count => 25), doctor.errors[:uid].join("; ")
 	end
 	...
 	test "doctor specialty must be between 0 and 50 characters" do
 		...
 		assert !doctor.save
 		assert_equal I18n.translate("errors.messages.too_long", :count => 50), doctor.errors[:speciality].join("; ")
 	end
 	
 	test "doctor fullname shortcut" do
 		doctor = doctors(:rebecca)
 		
 		assert_equal doctor.fullname, "#{s_('female_doctor_prefix|Dr.')} #{doctor.firstname} #{doctor.lastname}"
 	end
