...
class OdontogramFusionTest < ActionController::TestCase
  ...
  def change(entry = {}, **operation)
    post :create, params: { patient_id: @patient.id, request_id: SecureRandom.uuid, editor_id: @editor,
      ...
      odontogram_entry: { tooth: 11, paired_tooth: 21, category: 'fusion', surfaces: [] }.merge(entry) }.merge(operation), as: :json
  end
  ...
  test 'reversed pairs deduplicate while a different neighboring fusion remains independent' do
  ...
  test 'fusion and diastema on the same pair require explicit correction while other care remains' do
    change
    assert_response :created
    fusion = @patient.odontogram_entries.sole
    crown = @patient.odontogram_entries.create!(tooth: 21, category: 'crown', surfaces: [], recorded_by_name: 'Sample')
    change({ category: 'diastema' })
    assert_response :unprocessable_entity
    change({}, operation: 'remove', entry_id: fusion.id)
    assert_response :created
