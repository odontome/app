...
class Balance < ApplicationRecord
  ...
  scope :find_between, lambda { |starts_at, ends_at, practice_id|
    select('patients.uid, patients.id as patient_id, patients.firstname, patients.lastname, balances.id, balances.created_at, balances.amount, balances.notes, balances.currency')
