    ...
    @balance = balance_created_today
      ...
      @currency_unit = admin_user.first["currency_unit"]
	    ...
	    I18n.with_locale(@practice_locale) do
  			mail(:to => admin_user.first["email"], :subject => I18n.t("mailers.practice.daily_recap.subject", :date => l(@date.in_time_zone(@practice_timezone).to_date, :format => :day_and_date)))
  		end
  	end
  end
end
